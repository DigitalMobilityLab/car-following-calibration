...
The MATLAB files are stored directly in `Calibration/`; no additional `Source/` subfolder is required.
...
Run the code with `Calibration/` as the MATLAB current folder.
...
```text
Data/<PROJECT>/<DATABASE>/config.txt
```
...
`VehicleID=0` does **not** scan the folder automatically. It uses the `AllVehicleIDs` array defined near the beginning of `MAIN.m`.
...
```text
Data/<PROJECT>/<DATABASE>/Bounds/Model Class <ModelID>/parameters.txt
```
...
```text
Results/<PROJECT>/<DATABASE>/Model <ModelID>/VehicleID <VehicleID>/
```
